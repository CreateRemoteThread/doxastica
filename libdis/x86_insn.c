...
uint32_t x86_get_address( x86_insn_t *insn ) {
	...
	for (op_lst = (x86_oplist_t *)insn->operands; op_lst; op_lst = (x86_oplist_t *)op_lst->next ) {
...
unsigned char * x86_get_raw_imm( x86_insn_t *insn ) {
    ...
    {
        if ( insn->operands->next ) 
            {
		        if ( IS_PROPER_IMM( oplist_temp ) ) 
                    {
			            op = &(oplist_temp->op);
		            }
                    else 
                    {   
                        ...
                        if ( oplist_temp && IS_PROPER_IMM( oplist_temp2 ) ) 
                        {
			                op = &(oplist_temp2)->op;
		                }
                    }
