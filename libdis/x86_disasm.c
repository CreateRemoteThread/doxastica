...
unsigned int x86_disasm( unsigned char *buf, unsigned int buf_len,
	...
	insn->group = insn_none;
        ...
        if ( offset >= buf_len ) {
                ...
                x86_report_error(report_disasm_bounds, (int *)(long)buf_rva+offset);
                return 0;
        }
	...
	/* TODO: allow switching when more disassemblers are added */
        ...
        if (! size ) {
                x86_report_error(report_invalid_insn, (int *)(long)buf_rva+offset );
                return 0;
        }

        /* check if we overran the end of the buffer */
        if ( size > len ) {
                x86_report_error( report_insn_bounds, (int *)(long)buf_rva + offset );
