...
int x86_operand_foreach( x86_insn_t *insn, x86_operand_fn func, void *arg, 
	...
	for ( list = insn->operands; list; list = (x86_oplist_t *)list->next ) {
...
x86_op_t * x86_operand_2nd( x86_insn_t *insn ) {
    x86_oplist_t *oplist_temp = NULL;

	if ( insn->explicit_count < 2 ) {
		return NULL;
	}

    oplist_temp = (x86_oplist_t *)(insn->operands->next);

	return &(oplist_temp->op);
}

x86_op_t * x86_operand_3rd( x86_insn_t *insn ) {
    x86_oplist_t *oplist_temp = NULL;
    x86_oplist_t *oplist_temp2 = NULL;
	if ( insn->explicit_count < 3 ) {
		return NULL;
	}

    oplist_temp = (x86_oplist_t *)insn->operands->next;
    oplist_temp2 = (x86_oplist_t *)oplist_temp->next;

	return &(oplist_temp2->op);
}
