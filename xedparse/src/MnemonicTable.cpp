...
void MnemonicTranslateImplicitAlias(char* Mnemonic, Inst* Instruction)
{
            ...
            !_strnicmp(Mnemonic, "scas", 4))
    {
        if(Instruction->OperandCount >= 2)
        {
            ...
            if(Instruction->Operands[0].Type == OPERAND_MEM)
                Instruction->AddressSizeOverride = OpsizeToBits(RegGetSize(Instruction->Operands[0].Mem.BaseVal));
            else if(Instruction->Operands[1].Type == OPERAND_MEM)
                Instruction->AddressSizeOverride = OpsizeToBits(RegGetSize(Instruction->Operands[1].Mem.BaseVal));
        }
